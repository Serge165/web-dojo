...
import { SVG_BG_PRESETS, SVG_BG_PRESET_CATEGORIES } from "@/lib/svgBackgroundPresets";

export const SvgBackgroundPanel = ({ onApply }) => {
  const [presetId, setPresetId] = useState(SVG_BG_PRESETS[0].id);
  const [c1, setC1] = useState("#2563eb");
  const [c2, setC2] = useState("#f8fafc");
  const [size, setSize] = useState(SVG_BG_PRESETS[0].defaultSize);
  const [editing, setEditing] = useState("c1");

  const preset = SVG_BG_PRESETS.find((p) => p.id === presetId) || SVG_BG_PRESETS[0];
  const css = preset.build({ c1, c2, size });

  const selectPreset = (id) => {
    const p = SVG_BG_PRESETS.find((x) => x.id === id);
    if (!p) return;
    setPresetId(id);
  ...
  return (
    <div className="space-y-3" data-testid="svg-bg-panel">
      <div
        className="w-full h-20 rounded-md border border-[#332D22]"
        style={{ background: css }}
        data-testid="svg-bg-preview"
      />
      ...
      <select
        ...
        data-testid="svg-bg-preset-select"
      >
        {SVG_BG_PRESET_CATEGORIES.map((cat) => (
          <optgroup key={cat} label={cat}>
            {SVG_BG_PRESETS.filter((p) => p.category === cat).map((p) => (
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] uppercase tracking-wider text-[#948C79]">Size</span>
          <span className="text-[11px] font-mono text-[#A79C87]">{size}px</span>
        </div>
        <input
          type="range"
          min={Math.max(40, Math.round(preset.defaultSize * 0.3))}
          max={Math.round(preset.defaultSize * 2)}
          value={size}
          onChange={(e) => setSize(Number(e.target.value))}
          className="w-full"
          data-testid="svg-bg-size-input"
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          data-testid="svg-bg-edit-c1"
        >
          <div className="w-5 h-5 rounded border border-[#332D22]" style={{ background: c1 }} />
          <span className="text-[11px] font-mono text-[#A79C87]">Shape</span>
        </button>
        <button
      ...
      <div className="pt-2 border-t border-[#332D22]">
        <div className="text-[10px] uppercase tracking-wider text-[#948C79] mb-1">Editing {editing === "c1" ? "shape" : "base"} color</div>
        <ColorPicker
      ...
      <div className="pt-3 border-t border-[#332D22] space-y-2">
        <div className="bg-[#15130E] border border-[#332D22] rounded-md p-2 text-[11px] font-mono text-[#E4DECE] break-all max-h-24 overflow-y-auto" data-testid="svg-bg-css">
          {css}
        </div>
        <div className="grid grid-cols-2 gap-2">
          <button
