...
export const VariantPanel = ({ selected, onReplaceHtml }) => {
  if (!selected) {
    return <div className="text-[11px] text-gray-500">Select an element on the canvas to see its variants.</div>;
  }
  ...
  if (!info || !cat) {
    return (
      <div className="text-[11px] text-gray-500 space-y-2" data-testid="variant-panel-none">
  ...
  if (siblings.length === 0) {
    return <div className="text-[11px] text-gray-500">No other variants in "{cat.label}" yet.</div>;
  }
  return (
    <div className="space-y-2" data-testid="variant-panel">
      <div className="text-[11px] text-gray-400">
        <span className="text-white font-medium">{cat.label}</span> — {siblings.length} other variant{siblings.length === 1 ? "" : "s"}.
      ...
      <div className="space-y-1.5">
        {siblings.map((b) => (
          <button
            ...
            className="w-full text-left text-xs px-2.5 py-2 rounded border border-[#2B2B2B] bg-[#1F1F1F] hover:border-blue-500/60 hover:bg-[#232323] text-gray-200"
