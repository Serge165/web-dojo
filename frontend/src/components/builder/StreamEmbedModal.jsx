...
export const StreamEmbedModal = ({ open, onClose, onInsert }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="bg-[#1C1A15] border border-[#332D22] text-[#F1EDE2] max-w-4xl w-[92vw] max-h-[90vh] overflow-hidden p-0" data-testid="stream-embed-modal">
        ...
        <div className="grid grid-cols-[1fr_1fr] max-h-[calc(90vh-58px)]">
          <div className="border-r border-[#332D22] overflow-y-auto p-5 space-y-4">
            <div className="grid grid-cols-3 gap-2">
              <button onClick={() => setPlatform("twitch")} className={`py-2.5 rounded-lg text-sm font-medium border ${platform === "twitch" ? "bg-[#6441a5] border-[#6441a5] text-[#F1EDE2]" : "border-[#332D22] text-[#E4DECE] hover:border-[#6441a5]/60"}`} data-testid="stream-platform-twitch">Twitch</button>
              <button onClick={() => setPlatform("youtube")} className={`py-2.5 rounded-lg text-sm font-medium border ${platform === "youtube" ? "bg-[#ff0000] border-[#ff0000] text-[#F1EDE2]" : "border-[#332D22] text-[#E4DECE] hover:border-[#ff0000]/60"}`} data-testid="stream-platform-youtube">YouTube</button>
              <button onClick={() => setPlatform("discord")} className={`py-2.5 rounded-lg text-sm font-medium border ${platform === "discord" ? "bg-[#5865F2] border-[#5865F2] text-[#F1EDE2]" : "border-[#332D22] text-[#E4DECE] hover:border-[#5865F2]/60"}`} data-testid="stream-platform-discord">Discord</button>
            </div>
            ...
            {platform === "twitch" && (
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-[#948C79]">No API key needed — works on your real published site. It won't render inside Web Dojo's own Design canvas or in-app Preview (both sandbox embedded content in ways that block Twitch's parent-domain check); that's expected, not broken.</p>
              </div>
            )}

            {platform === "youtube" && (
              <div className="space-y-2">
                <div className="flex bg-[#15130E] border border-[#332D22] rounded-md p-0.5 text-[11px]">
                  <button onClick={() => setYtIsChannel(true)} className={`flex-1 py-1 rounded ${ytIsChannel ? "bg-[#242019] text-[#F1EDE2]" : "text-[#A79C87]"}`} data-testid="stream-yt-mode-channel">Channel (auto live)</button>
                  <button onClick={() => setYtIsChannel(false)} className={`flex-1 py-1 rounded ${!ytIsChannel ? "bg-[#242019] text-[#F1EDE2]" : "text-[#A79C87]"}`} data-testid="stream-yt-mode-video">Specific video ID</button>
                </div>
                <div>
            ...
            {platform === "discord" && (
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-[#948C79]">The server owner must enable "Server Widget" in Discord's Server Settings → Widget first, or this shows an error instead of the member list.</p>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col overflow-hidden bg-[#0e0e10]">
            <div className="px-4 py-2 border-b border-[#332D22] bg-[#1C1A15] flex justify-between items-center">
              ...
              <button onClick={insert} className="text-xs px-3 py-1.5 rounded bg-emerald-600 hover:bg-emerald-500 text-[#F1EDE2] font-medium" data-testid="stream-insert">Insert onto canvas</button>
            </div>
            <iframe
