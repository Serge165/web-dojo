...
import { Code2, MousePointer2, Download, Upload, Save, FolderOpen, ChevronDown } from "lucide-react";
...
export const TopBar = ({
  ...
  return (
    <header className="h-14 flex-none border-b border-[#2B2B2B] bg-[#141414] flex items-center justify-between px-4" data-testid="top-bar">
      <div className="flex items-center gap-3">
        ...
        <input
          ...
          className="bg-transparent text-sm text-gray-200 border border-transparent hover:border-[#2B2B2B] focus:border-blue-500 rounded px-2 py-1 outline-none w-56"
      ...
      <div className="flex items-center gap-2 relative">
        ...
        {importOpen && (
          <div className="absolute right-40 top-11 w-96 bg-[#141414] border border-[#2B2B2B] rounded-md p-3 z-50 shadow-2xl" data-testid="import-panel">
            ...
            <button
              ...
              className="w-full text-xs py-1.5 rounded bg-[#1F1F1F] border border-[#2B2B2B] text-gray-200 hover:bg-[#2B2B2B] mb-2"
              data-testid="import-file-btn"
            >Choose .html file…</button>
            ...
            <textarea
              ...
              className="w-full bg-[#0D0D0D] border border-[#2B2B2B] rounded p-2 text-xs font-mono text-white outline-none focus:border-blue-500"
              data-testid="import-paste-textarea"
            />
            <div className="flex justify-end gap-2 mt-2">
              <button
                onClick={() => setImportOpen(false)}
                className="text-xs px-3 py-1 rounded bg-[#1F1F1F] text-gray-300 border border-[#2B2B2B]"
              >Cancel</button>
              <button
        ...
        {exportOpen && (
          <div className="absolute right-24 top-11 w-52 bg-[#141414] border border-[#2B2B2B] rounded-md p-1 z-50 shadow-2xl" data-testid="export-menu">
            <button
              onClick={() => { downloadStandalone(project); setExportOpen(false); }}
              className="w-full text-left text-xs px-2 py-1.5 rounded hover:bg-[#1F1F1F] text-gray-200"
              data-testid="export-standalone"
            >Standalone .html (inline CSS)</button>
            <button
              onClick={() => { downloadZip(project); setExportOpen(false); }}
              className="w-full text-left text-xs px-2 py-1.5 rounded hover:bg-[#1F1F1F] text-gray-200"
        ...
        <button
          onClick={onSave}
          className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md bg-blue-600 hover:bg-blue-500 text-white"
