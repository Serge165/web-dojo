...
export const buildFormHtml = (config) => {
  ...
  const script = `<script>(function(){var fs=document.querySelectorAll('form[data-form-id="${config.id}"]');Array.prototype.forEach.call(fs,function(f){if(f.__wdBound){return;}f.__wdBound=1;f.addEventListener('submit',function(ev){ev.preventDefault();var fd=new FormData(f);fd.append('_wd_page',location.href);fd.append('_wd_title',document.title);fd.append('_wd_project',window.__WD_PROJECT_ID||'');var b=f.querySelector('[type=submit],button');if(b){b.disabled=true;}fetch(f.getAttribute('action'),{method:'POST',body:fd,headers:{'Accept':'application/json'}}).then(function(r){if(!r.ok){throw 0;}f.innerHTML='<div style="padding:16px 0;color:#16a34a;font-weight:600;font-size:15px;font-family:Inter,system-ui,sans-serif;">'+(f.getAttribute('data-success')||'Thanks!')+'</div>';}).catch(function(){if(b){b.disabled=false;}alert('Sorry, something went wrong. Please try again.');});});});})();<\/script>`;
