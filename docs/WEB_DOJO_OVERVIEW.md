...
- **Access control is inconsistent across dashboards.** E-commerce endpoints require a password-derived `X-Dashboard-Token`. Submissions (`/api/submissions`) and site-visit analytics (`/api/projects/{project_id}/analytics`) require nothing beyond knowing the `project_id` — no password, no token. Anyone who can guess or find a `project_id` can read that project's form submissions and traffic stats today.
